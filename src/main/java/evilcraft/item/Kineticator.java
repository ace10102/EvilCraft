...
public class Kineticator extends ConfigurableDamageIndicatedItemFluidContainer {
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer, List list, boolean par4) {
        ...
        L10NHelpers.addStatusInfo(list, ItemHelpers.isActivated(itemStack), getUnlocalizedName() + ".info" + (isRepelling(itemStack) ? ".repelling" : "") + ".attraction");
        list.add(EnumChatFormatting.BOLD + L10NHelpers.localize(getUnlocalizedName() + ".info.area", getArea(itemStack)));
    }
    ...
    private void kineticate(ItemStack itemStack, World world, Entity entity) {
        if(ItemHelpers.isActivated(itemStack) && (getFluid(itemStack) != null || (entity instanceof EntityPlayer && canConsume(1, itemStack, (EntityPlayer)entity))) && (entity == null || !entity.isSneaking())) {
            ...
            if(0 == world.getWorldTime() % KineticatorConfig.tickHoldoff) {
                ...
                for(Entity moveEntity : entities) {
                    if(repelling || (moveEntity instanceof EntityItem && ((EntityItem)moveEntity).delayBeforeCanPickup == 0 && canKineticateItem(((EntityItem)moveEntity).getEntityItem())) || (moveEntity instanceof EntityXPOrb)) {
                        ...
                        if((repelling || d > 0.5D) && (usage == 0 || (this.drain(itemStack, usage, false) != null) || (isPlayer && this.canConsume(usage, itemStack, (EntityPlayer)entity)))) {
    ...
    protected void showEntityMoved(World world, Entity player, Entity entity, double dx, double dy, double dz) {
        ...
        EntityBlurFX blur = new EntityBlurFX(world, entity.posX, entity.posY, entity.posZ, scale, -dx, -dy, -dz, red, green, blue, ageMultiplier);
