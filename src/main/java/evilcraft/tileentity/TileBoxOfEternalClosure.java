...
public class TileBoxOfEternalClosure extends EvilCraftTileEntity {
    ...
    public void updateTileEntity() {
        ...
        if(spirit == null && !worldObj.isRemote) {
            if(target != null || (WorldHelpers.efficientTick(getWorldObj(), TICK_MODULUS, xCoord, yCoord, zCoord) && findNextEntity())) {
                pullEntity();
            }
        }
        if(worldObj.isRemote && target != null) {
            EvilCraft.proxy.playSound(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D, "boxBeam", 0.1F + worldObj.rand.nextFloat() * 0.9F, 0.1F + worldObj.rand.nextFloat() * 0.9F);
        }
        close(false);
    }

    private boolean findNextEntity() {
        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord, yCoord, zCoord).expand(TARGET_RADIUS, TARGET_RADIUS, TARGET_RADIUS);
    ...
    private void pullEntity() {
        ...
        if(target != null) {
            ...
            if(target.isDead || !target.isFrozen()) {
                ...
                if(target.boundingBox.expand(ABSORB_RADIUS, ABSORB_RADIUS, ABSORB_RADIUS).intersectsWith(getBlock().getCollisionBoundingBoxFromPool(worldObj, xCoord, yCoord, zCoord))) {
                    closing = true;
                    close(true);
    ...
    public void releaseSpirit() {
        ...
        spirit.setRemainingLife(MathHelper.getRandomIntegerInRange(worldObj.rand, VengeanceSpirit.REMAININGLIFE_MIN, VengeanceSpirit.REMAININGLIFE_MAX));
    ...
    protected void setSpiritInstance(EntityLivingBase spiritInstance) {
        ...
        if(old != spiritInstance) {
            if(old == null) {
                if(!initial) {
                    getWorldObj().playSoundEffect(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D, "random.chestclosed", 0.5F, getWorldObj().rand.nextFloat() * 0.1F + 0.9F);
                }
            }
            sendUpdate();
        }
