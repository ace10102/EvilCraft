...
public class TileSpiritPortal extends EvilCraftTileEntity {
    ...
    public void updateTileEntity() {
        ...
        if(worldObj.isRemote) {
            ...
            if(worldObj.rand.nextInt(5) == 0) {
                EvilCraft.proxy.playSound(xCoord + 0.5F, yCoord + 0.5F, zCoord + 0.5F, "vengeanceBeam", 0.5F + worldObj.rand.nextFloat() * 0.2F, 1.0F);
            }
        ...
        if(!worldObj.isRemote && Configs.isEnabled(OriginsOfDarknessConfig.class)) {
            for(EntityItem entityItem : (List<EntityItem>)worldObj.getEntitiesWithinAABB(EntityItem.class,
                    AxisAlignedBB.getBoundingBox(this.xCoord - 0.5D, this.yCoord - 0.5D, this.zCoord - 0.5D, this.xCoord + 1.5D, this.yCoord + 1.5D, this.zCoord + 1.5D))) {
                if(entityItem.getEntityItem().getItem() instanceof ItemBook) {
                    Entity entity = new EntityItem(worldObj, entityItem.posX, entityItem.posY, entityItem.posZ, new ItemStack(OriginsOfDarkness.getInstance(), entityItem.getEntityItem().stackSize));
    ...
    private void showNewBlurParticle() {
        ...
        EntityBlurFX blur = new EntityBlurFX(worldObj, xCoord + 0.5F, yCoord + 0.5F, zCoord + 0.5F, scale, rand.nextFloat() * 0.2F - 0.1F, rand.nextFloat() * 0.2F - 0.1F, rand.nextFloat() * 0.2F - 0.1F, red, green, blue, ageMultiplier);
