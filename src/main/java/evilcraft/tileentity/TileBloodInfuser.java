...
public class TileBloodInfuser extends TileWorking<TileBloodInfuser, MutableInt> {
    ...
    public TileBloodInfuser() {
        super(SLOTS, BloodInfuser.getInstance().getLocalizedName(), LIQUID_PER_SLOT, TileBloodInfuser.TANKNAME, ACCEPTED_FLUID);
        infuseTicker = addTicker(new TickComponent<TileBloodInfuser, ITickAction<TileBloodInfuser>>(this, INFUSE_TICK_ACTIONS, SLOT_INFUSE));
        addTicker(new TickComponent<TileBloodInfuser, ITickAction<TileBloodInfuser>>(this, EMPTY_IN_TANK_TICK_ACTIONS, SLOT_CONTAINER, false));
        ...
        recipeCache = new SingleCache<Triple<ItemStack, FluidStack, Integer>,
            ...
            public IRecipe<ItemFluidStackAndTierRecipeComponent, ItemStackRecipeComponent, DurationXpRecipeProperties> getNewValue(Triple<ItemStack, FluidStack, Integer> key) {
                ItemFluidStackAndTierRecipeComponent recipeInput = new ItemFluidStackAndTierRecipeComponent(key.getLeft(), key.getMiddle(), -1);
            ...
            public boolean isKeyEqual(Triple<ItemStack, FluidStack, Integer> cacheKey, Triple<ItemStack, FluidStack, Integer> newKey) {
                return cacheKey == null || newKey == null || (ItemStack.areItemStacksEqual(cacheKey.getLeft(), newKey.getLeft())
                        && FluidStack.areFluidStackTagsEqual(cacheKey.getMiddle(), newKey.getMiddle()) && cacheKey.getRight().equals(newKey.getRight()));
            }
        });
    }
    ...
    public IRecipe<ItemFluidStackAndTierRecipeComponent, ItemStackRecipeComponent, DurationXpRecipeProperties> getRecipe(ItemStack itemStack) {
        return recipeCache.get(Triple.of(itemStack == null ? null : itemStack.copy(), getTank().getFluid() == null ? null : getTank().getFluid().copy(), getTier()));
    }

    @Override
    public boolean canConsume(ItemStack itemStack) {
        // Empty bucket
        if(itemStack.getItem() == Items.bucket && this.getTank().getFluidAmount() >= FluidContainerRegistry.BUCKET_VOLUME)
            return true;
        ...
        if(itemStack.getItem() instanceof IFluidContainerItem) {
            ...
            if(fluidStack == null) {
                ...
                if(getTank().canTankAccept(fluidStack.getFluid()) && fluidStack.amount < container.getCapacity(itemStack)) {
                    return true;
                }
            }
        }
        // Valid custom recipe
        IRecipe<ItemFluidStackAndTierRecipeComponent, ItemStackRecipeComponent, DurationXpRecipeProperties> recipe = getRecipe(itemStack);
        if(recipe != null)
            return true;
