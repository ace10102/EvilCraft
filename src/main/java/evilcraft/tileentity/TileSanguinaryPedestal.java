...
public class TileSanguinaryPedestal extends TankInventoryTileEntity {
    ...
    public TileSanguinaryPedestal() {
        super(0, PurifierConfig._instance.getNamedId(), 1, FluidContainerRegistry.BUCKET_VOLUME * TANK_BUCKETS, SanguinaryPedestalConfig._instance.getNamedId() + "tank", FLUID);
    }
        ...
        if(!getWorldObj().isRemote) {
            ...
            while(!getTank().isFull() && actions > 0) {
                ...
                if(block == BloodStainedBlock.getInstance()) {
                    BloodStainedBlock.UnstainResult result = BloodStainedBlock.getInstance().unstainBlock(getWorldObj(), location, getTank().getCapacity() - getTank().getFluidAmount());
                    if(result.amount > 0) {
            ...
            if(!getTank().isEmpty()) {
                for(ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(!getTank().isEmpty() && tile instanceof IFluidHandler) {
                        ...
                        if(handler.canFill(direction.getOpposite(), getTank().getFluidType()) && handler.fill(direction.getOpposite(), fluidStack, false) > 0) {
