...
public class EntityBroomConfig extends EntityConfig {
    ...
    /**
     * Maximum number of blocks the position on the client can differ from the position sent by the server
     * (note: this value should take into account the round trip delay between client and server because the code does not handle that.
     * So the position sent by the server will always be an "old" position).
     */
    ...
    /**
     * The maximum number of blocks the client can move the position of the broom at a time,
     * once we notice there is a desync between the position on the client and server.
     */
