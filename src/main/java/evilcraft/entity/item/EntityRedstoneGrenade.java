...
public class EntityRedstoneGrenade extends EntityThrowable implements IConfigurable {
    ...
    /**
     * Maps a side number to the offset of the X, Y and Z coordinates where the redstone block will be spawned when the redstone grenade hits the ground
    ...
    protected void onImpact(MovingObjectPosition pos) {
        ...
        if(worldObj.isAirBlock(pos.blockX + dx, pos.blockY + dy, pos.blockZ + dz)) {
            if(Configs.isEnabled(InvisibleRedstoneBlockConfig.class)) {
                worldObj.setBlock(pos.blockX + dx, pos.blockY + dy, pos.blockZ + dz, InvisibleRedstoneBlock.getInstance());
            }
