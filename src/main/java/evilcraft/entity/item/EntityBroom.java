...
public class EntityBroom extends Entity implements IConfigurable {
    ...
    /**
     * Maximum angle of the broom between the XZ-plane and the Y-axis (in degrees, -90 = completely up, +90 = completely down)
     * This limits the angle under which the player can move up or down
     */
    public static final float MAX_ANGLE = 45.0F;
    /**
     * Minimum angle of the broom between the XZ-plane and the Y-axis (in degrees, -90 = completely up, +90 = completely down)
     * This limits the angle under which the player can move up or down
     */
    ...
    /**
     * This variable holds the last hover offset that was set by this broom during its last update.
     */
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch, int posRotationIncrements) {
        ...
        /**
         * If the player on the broom is the same as the client player, then make some corrections for its position based on what the server sent us
         */
    ...
    public void onUpdate() {
        ...
        if(!worldObj.isRemote && riddenByEntity == null && lastMounted != null) {
            ...
            /*
             * TODO: if we ever have the problem that a player can dismount without getting the broom back in his inventory and
             * removing the entity from the world its probably because of this next line of code because onUpdate() is called AFTER
             * the player is dismounted, thus lastMounted is not updated before the player dismounts and thus the dismounting code
             * is never executed
             */
    ...
    /**
     * Called on the server side for all players or on the client side when the player mounted on the broom is the local player,
     * so movement is as smooth as possible.
     */
