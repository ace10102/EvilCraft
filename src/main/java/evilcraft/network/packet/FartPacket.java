...
/**
 * Instances of this class are sent between client and server to notify both about a farts which are spawned.
 * @author immortaleeb
 */
public class FartPacket extends PacketCodec {
    ...
    public boolean hasRainbowFart(EntityPlayer player) {
        return player.getGameProfile() != null && ALLOW_RAINBOW_FARTS.contains(player.getGameProfile().getId());
    }
    ...
    public void actionServer(World world, EntityPlayerMP player) {
        if(GeneralConfig.farting) {
            ...
            PacketHandler.sendToAllAround(new FartPacket(player), LocationHelpers.createTargetPointFromEntityPosition(player, FART_RANGE));
        }
    }
}