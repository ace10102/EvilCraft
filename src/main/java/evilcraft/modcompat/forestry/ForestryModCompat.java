    ...
    public void onInit(IInitListener.Step step) {
        if(step == IInitListener.Step.INIT) {
            ...
            if(Configs.isEnabled(UndeadSaplingConfig.class)) {
                FMLInterModComms.sendMessage(getModID(), "add-farmable-sapling", "farmArboreal@" + Block.blockRegistry.getNameForObject(UndeadSaplingConfig._instance.getBlockInstance()) + ".0");
            }

            // Add dark gem to the miner backpack.
            if(Configs.isEnabled(DarkGemConfig.class)) {
                FMLInterModComms.sendMessage(getModID(), "add-backpack-items", "miner@" + Item.itemRegistry.getNameForObject(DarkGem.getInstance()) + ":*");
            }

            // Add poison sac to hunter backpack.
            if(Configs.isEnabled(PoisonSacConfig.class)) {
                FMLInterModComms.sendMessage(getModID(), "add-backpack-items", "hunter@" + Item.itemRegistry.getNameForObject(PoisonSacConfig._instance.getItemInstance()) + ":*");
            }

            // Add undead log to forester backpack.
            if(Configs.isEnabled(UndeadLogConfig.class)) {
                FMLInterModComms.sendMessage(getModID(), "add-backpack-items", "forester@" + Block.blockRegistry.getNameForObject(UndeadLogConfig._instance.getBlockInstance()) + ":*");
            }
