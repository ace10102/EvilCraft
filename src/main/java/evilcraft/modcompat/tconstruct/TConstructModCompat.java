...
public class TConstructModCompat implements IModCompat {
    ...
    public void onInit(IInitListener.Step step) {
        if(step == IInitListener.Step.POSTINIT) {
            RegistryManager.getRegistry(IBloodChestRepairActionRegistry.class).register(new TConstructToolRepairTickAction());
            if(Configs.isEnabled(EnderTearConfig.class)) {
                Smeltery.addMelting(FluidType.getFluidType("Ender"), new ItemStack(EnderTearConfig._instance.getItemInstance()), 0, EnderTearConfig.mbLiquidEnder);
            }
        }
    }
