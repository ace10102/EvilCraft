...
public class WorldGeneratorUndeadTree extends WorldGenerator {
    ...
    public boolean growTree(World world, Random rand, int x, int y, int z) {
        ...
        if(y >= 1 && y + treeHeight + 1 <= worldHeight) {
            ...
            if((block != null && block.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, ((BlockSapling)sapling))) && y < worldHeight - treeHeight - 1) {
                    ...
                    if(yOffset >= 0 & yOffset < worldHeight) {
                        for(xOffset = x - radius; xOffset <= x + radius; ++xOffset) {
                            for(zOffset = z - radius; zOffset <= z + radius; ++zOffset) {
                                ...
                                if(block != null && !(block.isLeaves(world, xOffset, yOffset, zOffset) || block == Blocks.air || block.canBeReplacedByLeaves(world, xOffset, yOffset, zOffset))) {
                                    return false;
                                }
                            }
                        }
                ...
                if(block != null) {
                        ...
                        for(xOffset = x - center; xOffset <= x + center; ++xOffset) {
                            ...
                            for(zOffset = z - center; zOffset <= z + center; ++zOffset) {
                                ...
                                if(((xPos != center | zPos != center) || rand.nextInt(2) != 0 && var12 != 0) &&
                                        (block == null || block.isLeaves(world, xOffset, yOffset, zOffset) || block == Blocks.air || block.canBeReplacedByLeaves(world, xOffset, yOffset, zOffset))) {
                    ...
                    for(yOffset = 0; yOffset < treeHeight; ++yOffset) {
                        ...
                        if(block == null || block == Blocks.air || block.isLeaves(world, x, y + yOffset, z) || block.isReplaceable(world, x, y + yOffset, z)) {
