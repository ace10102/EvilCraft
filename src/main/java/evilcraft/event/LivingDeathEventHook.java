...
public class LivingDeathEventHook {
    ...
    private void bloodObtainEvent(LivingDeathEvent event) {
        ...
        if(e instanceof EntityPlayerMP && !e.worldObj.isRemote && event.entityLiving != null && Configs.isEnabled(BloodExtractorConfig.class)) {
            float boost = 1.0F;
            EntityPlayerMP player = (EntityPlayerMP)e;
            if(Configs.isEnabled(VeinSwordConfig.class) && player.getHeldItem() != null && player.getHeldItem().getItem() == VeinSword.getInstance()) {
    ...
    private void bloodStainedBlockEvent(LivingDeathEvent event) {
        if(event.source == DamageSource.fall && Configs.isEnabled(BloodStainedBlockConfig.class) && !(event.entity instanceof VengeanceSpirit)) {
            ...
            if(BloodStainedBlock.getInstance().canSetInnerBlock(block, event.entity.worldObj, x, y, z) || block == BloodStainedBlock.getInstance()) {
                if(!event.entity.worldObj.isRemote) {
                    // Transform block into blood stained version
                    BloodStainedBlock.getInstance().stainBlock(event.entity.worldObj, new Location(x, y, z), (int)(BloodStainedBlockConfig.bloodMBPerHP * event.entityLiving.getMaxHealth()));
                } else {
                    // Init particles
    ...
    private void vengeanceEvent(LivingDeathEvent event) {
        if(event.entityLiving != null) {
            ...
            if(!world.isRemote && !(world instanceof FakeWorld) && world.difficultySetting != EnumDifficulty.PEACEFUL
                    && Configs.isEnabled(VengeanceSpiritConfig.class) && VengeanceSpirit.canSustain(event.entityLiving)
    ...
    private void dropHumanoidFleshEvent(LivingDeathEvent event) {
        if(event.entityLiving instanceof EntityPlayerMP && Configs.isEnabled(WerewolfFleshConfig.class)
                && !event.entityLiving.worldObj.isRemote && event.entityLiving.worldObj.rand.nextInt(WerewolfFleshConfig.humanoidFleshDropChance) == 0) {
