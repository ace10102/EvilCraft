...
/**
 * A config that refers to a {@link IConfigurable}.
 * Every unique {@link IConfigurable} must have one unique extension of this class.
 * This contains several configurable settings and properties that can also be set in the config file.
 * @author rubensworks
...
public abstract class ExtendedConfig<C extends ExtendedConfig<C>> implements Comparable<ExtendedConfig<C>>, IInitListener {
    ...
    /**
     * Create a new config
     * @param enabled If this should is enabled by default. If this is false, this can still be enabled through the config file.
    ...
    /**
     * Generate the list of ConfigProperties by checking all the fields with the ConfigurableProperty annotation.
    ...
    private void generateConfigProperties() throws IllegalArgumentException, IllegalAccessException {
        for(Field field : this.getClass().getDeclaredFields()) {
            if(field.isAnnotationPresent(ConfigurableProperty.class)) {
                ...
                ConfigProperty configProperty = new ConfigProperty(category, getConfigPropertyPrefix() + "." + field.getName(), field.get(null), annotation.comment(), new ConfigPropertyCallback(changedCallback), annotation.isCommandable(), field);
    ...
    /**
     * This method will by default just return null. If it returns something else, this config will assume that the object
     * that is returned is the unique sub-instance for the configurable. This is only called once.
    ...
    /**
     * If the target should be hard-disabled, this means no occurrence in the config file, total ignorance.
