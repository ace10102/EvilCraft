...
public class CubeDetector {
    ...
    /**
     * The size is the inner size -1 on each dimension, so 2x2x2 is one block in the middle open, with edge blocks on the side forming a 3x3x3.
    ...
    protected boolean isValidLocation(World world, ILocation location, IValidationAction action) {
        Block block = world.getBlock(location.getCoordinates()[0], location.getCoordinates()[1], location.getCoordinates()[2]);
    ...
    protected boolean isAir(World world, ILocation location) {
        return world.isAirBlock(location.getCoordinates()[0], location.getCoordinates()[1], location.getCoordinates()[2]);
    }
     ...
     * @param startLocation The location to start looking from.
     * @param dimension The dimension to navigate in.
     * @param max If the direction to look in for the given dimension should be positive otherwise negative.
     * @return The found border.
     */
    ...
    /**
     * Check if a given location is valid, taking into account the edges of the structure,
     * so that we know which locations should be borders, and which ones should be air.
    ...
    /**
     * Validate if the block at the given location conforms with the {@link AllowedBlock} conditions.
     * @param world The world.
     ...
     * To initiate the recursion, call this with an empty accumulatedCoordinates array,
     * it will then simulate for loop for every dimension between the start and stop coordinate for that dimension.
     * When the accumulatedCoordinates size equals the desired amount of dimensions, this recursion will enter a leaf and check the conditions for that location.
     * @param world The world.
     * @param dimensionEgdes The edges per dimension. [dimension][start=0 | stop=1]
    ...
    protected boolean validateDimensionEdges(World world, final int[][] dimensionEgdes, final boolean valid, final IValidationAction action) {
        ...
        boolean minimumValid = coordinateRecursion(world, dimensionEgdes, new ILocationAction() {
            ...
            public boolean run(World world, ILocation location) {
                ...
                return !(valid && !validateAllowedBlockConditions(world, location)) && validateLocationInStructure(world, dimensionEgdes, location, action);
            }
        });
     ...
     * @param valid True if the structure should be validated, false if it should be invalidated.
     * @param changeState If the post-validate actions should be called, and thus potentially change the world block states.
     * @return The size of the found structure. Note that a size in a dimension here starts counting from 0,
     * so a 1x1x1 structure (=1 block) will return a size of 0 in each dimension.
     */
    ...
    public Size detect(World world, ILocation startLocation, boolean valid, IValidationAction action, boolean changeState) {
        ...
        if(getMinimumSize() != NULL_SIZE) {
            ...
            // TODO: the 'valid' condition might be impossible to have influence if all structure handling goes according to plan.
