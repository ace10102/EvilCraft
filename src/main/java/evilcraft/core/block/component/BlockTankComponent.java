...
public class BlockTankComponent<T extends BlockContainer & IBlockTank> {
     ...
     */
    public boolean onBlockActivatedTank(World world, int x, int y, int z, EntityPlayer player, int side, float motionX, float motionY, float motionZ) {
                ...
                if(itemStack.getItem() instanceof IFluidContainerItem) {
                    ...
                    if(player.isSneaking()) { // Drain from tank into item.
                        if(tile.getTank().getFluidAmount() > 0) {
                            FluidStack fluidStackTemp = new FluidStack(tile.getTank().getFluid().getFluid(), FluidContainerRegistry.BUCKET_VOLUME);
                        ...
                        FluidStack fluidStackTemp = new FluidStack(fluidStack.getFluid(), Math.min(FluidContainerRegistry.BUCKET_VOLUME, fluidStack.amount));
