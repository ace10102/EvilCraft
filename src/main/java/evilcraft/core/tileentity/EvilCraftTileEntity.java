 ...
 * Every time the counter reaches zero, the backoff will be reset and an update packet will be sent if one has been queued.
 * @author rubensworks
 */
public class EvilCraftTileEntity extends TileEntity {
     ...
     * This will always send lag-safe updates, so calling this many times per tick will not cause multiple packets to be sent, more info in the class javadoc.
     */
     ...
     * This does the same as {@link EvilCraftTileEntity#sendUpdate()} but will ignore the update backoff.
     */
     ...
     * @return The NBT tag that is created with the {@link EvilCraftTileEntity#writeToNBT(NBTTagCompound)} method.
     */
    ...
    /**
     * Get the current rotation of this tile entity. Default is {@link ForgeDirection#NORTH}.
     * @return The rotation.
     */
    ...
    /**
     * Set the rotation of this tile entity. Default is {@link ForgeDirection#NORTH}.
    ...
    /**
     * This method is called when the tile entity receives an update (ie a data packet) from the server.
     * If this tile entity uses NBT, then the NBT will have already been updated when this method is called.
     */
