 ...
 * It will then tick for the given tile and slot and check the current item inside the given slot.
 * It will then take the class of that item and lookup the item class inside that map to get the correct {@link ITickAction} on which it will call the methods.
 * @author rubensworks
...
public class TickComponent<C extends EvilCraftTileEntity, T extends ITickAction<C>> {
     ...
     * @param tickActions The collection of actions this ticker can perform. It must map the item class to an extension of {@link ITickAction}.
