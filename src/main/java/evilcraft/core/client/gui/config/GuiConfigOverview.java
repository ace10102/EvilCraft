...
public class GuiConfigOverview extends GuiConfig {
    ...
    public GuiConfigOverview(GuiScreen parentScreen) {
        super(parentScreen, getConfigElements(), Reference.MOD_ID, false, false, GuiConfig.getAbridgedConfigPath(ConfigHandler.getInstance().getConfig().toString()));
    }
    ...
    public static class ExtendedCategoryEntry extends CategoryEntry {
        ...
        protected GuiScreen buildChildScreen() {
            ...
            this.category = ConfigurableTypeCategory.valueOf(configElement.getName().replaceAll(" ", "").toUpperCase(Locale.ENGLISH));

            // Get all the elements inside this category
            List<IConfigElement> elements = (new ConfigElement(ConfigHandler.getInstance().getConfig().getCategory(category.toString()))).getChildElements();
            return new GuiConfig(this.owningScreen, elements, this.owningScreen.modID, category.toString(), this.configElement.requiresWorldRestart()
                    || this.owningScreen.allRequireWorldRestart, this.configElement.requiresMcRestart()
                    || this.owningScreen.allRequireMcRestart, GuiConfig.getAbridgedConfigPath(ConfigHandler.getInstance().getConfig().toString()));
        }
    }
}