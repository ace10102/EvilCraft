...
public class ItemBlockFluidContainer extends ItemBlockNBT implements IFluidContainerItem {
    ...
    protected void setFluid(ItemStack container, FluidStack fluidStack) {
        ...
        if(fluidStack == null || fluidStack.amount <= 0) {
            ...
            fluidTag.removeTag("Empty"); // Make sure that after this block is placed the fluid inside is correctly recognized.
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer, List list, boolean par4) {
        ...
        if(block.isActivatable()) {
            L10NHelpers.addStatusInfo(list, block.isActivated(itemStack, entityPlayer.worldObj, entityPlayer), getUnlocalizedName() + ".info.autoSupply");
        }
    }
}