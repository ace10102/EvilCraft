...
public class BloodInfuserRecipeTypeHandler extends SuperRecipeTypeHandler {
    ...
    protected ItemStack handleRecipe(Element input, Element output, Element properties) throws XmlRecipeException {
        ...
        if(item instanceof ItemStack) {
            recipeComponent = new ItemFluidStackAndTierRecipeComponent((ItemStack)item, new FluidStack(fluid, inputAmount), tier);
        } else {
            recipeComponent = new ItemFluidStackAndTierRecipeComponent((String)item, new FluidStack(fluid, inputAmount), tier);
        }

        ItemStack outputStack = (ItemStack)getItem(outputItem);
        BloodInfuser.getInstance().getRecipeRegistry().registerRecipe(recipeComponent, new ItemStackRecipeComponent(outputStack), new DurationXpRecipeProperties(duration, xp));
        return outputStack;
    }
}